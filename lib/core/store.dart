...
class Store {
  ...
  String get silverSubscriptionId => _cluster.env.products.firstWhere((e) => e.name == "silver").cart;
  ...
  bool isConsumableById(String id) => _cluster.env.products.indexWhere((e) => e.cart == id && e.type == "consumable") >= 0;
  ...
  testUpdate(String str){
    // this.message = str;
    this.message = _cluster.env.name;
    this._notify();
  }
  ...
  Future<void> doRestore() async {
    ...
    for (var e in listOfPurchaseItem) {
      if (_cluster.boxOfPurchase.values.where((o) => o.purchaseId == e.purchaseID).length == 0){
        _cluster.boxOfPurchase.add(
          PurchaseType(
  ...
  Future<void> _handleDeliverPurchase(PurchaseDetails item) async {
    ...
    if (this.isConsumableById(item.productID)) {
      if (item.pendingCompletePurchase && item.status == PurchaseStatus.purchased){
        ...
        if (_cluster.boxOfPurchase.values.where((o) => o.purchaseId == item.purchaseID).length == 0){
          _cluster.boxOfPurchase.add(
            PurchaseType(
